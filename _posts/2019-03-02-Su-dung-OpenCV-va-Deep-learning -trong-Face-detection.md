...
Bây giờ sẽ là demo của việc ứng dụng của detect khuôn mặt sử dụng kỷ thuật deep learning trong OpenCV. Tất nhiên xử lý cho video thi ta coi như xử lý chuỗi ảnh và không có kỹ thuật gì thêm nữa.
### code
```python
# import the necessary packages
...
Chúng ta sẽ cần thêm 2 gói sau để xử lý video
```python
import VideoStream
import imutils
import time
...
Ở ví dụ trên một ảnh đơn chúng ta phải pass đường dẫn ảnh đầu vào, còn ở đây chúng ta sẽ đọc từ webcam.
```python
vs = VideoStream(src=0).start()
time.sleep(2.0)
```
